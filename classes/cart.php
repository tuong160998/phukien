	...
	*/
	class cart
	{
		...
		public function add_to_cart($id, $quantity)
		{
			...
			$query_cart = "SELECT * FROM tbl_cart WHERE productId = '$id' ";
			$result_cart = $this->db->select($query_cart);
			if(!empty($result_cart)){
				$result_cart = $this->db->select($query_cart)->fetch_assoc();
				$incease_quantity = $result_cart['quantity'] + $quantity;
			}else{
				$result_cart = [];
				$incease_quantity = $quantity;
			}
			...
			if($result['product_remain']>$quantity){
				if(empty($result_cart)){
					...
					$update_cart = $this->db->insert($query_insert);
				}else{
					$query_update = "UPDATE tbl_cart SET quantity='$incease_quantity' WHERE productId='$id'";
